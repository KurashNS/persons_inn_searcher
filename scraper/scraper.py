...
import ua_generator
from scraper.proxy import get_proxy_connector
...
class BaseClient:
    def __init__(self, person: Person, session: ClientSession, headers: dict[str: str], logger: Logger) -> None:
        self._person = person
...
class NalogRuClient(BaseClient):
    ...
    async def _create_search_inn_request(self) -> str:
        ...
        self._logger.info(f'Create search INN request response (nalog.ru | person ID: {self._person_id}): {create_request_resp_json}')

        try:
            captcha_required = create_request_resp_json["captchaRequired"]
            self._logger.info(f'CAPTCHA required (nalog.ru | person ID: {self._person_id}): {captcha_required}')
    ...
    @retry(
        retry=retry_if_exception_type(ClientError), wait=wait_fixed(3) + wait_random(min=0, max=2),
        sleep=asyncio.sleep, stop=stop_after_attempt(3), reraise=True
    )
...
async def search_inn_(person: Person, logger: Logger) -> Person:
    async with _semaphore:
        inn = str()
        search_status = str()

        async with ClientSession(connector=get_proxy_connector(), raise_for_status=True) as session:
            try:
...
if __name__ == '__main__':
    ...
    async def test():
        ...
        tasks = [search_inn_(person=person, logger=logger) for person in persons_list]
